            ...
            <li class="flex flex-col md:flex-row items-start bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
                ...
                <div class="flex-1 mt-4 md:mt-0 md:ml-4">
                    ...
                    <div class="mt-3 flex space-x-4 text-gray-600 dark:text-gray-300">
                        ...
                        $isFormOpen = isset($_GET['show_comment_form']) && $_GET['show_comment_form'] == $song['id'];
                        $formToggleUrl = $isFormOpen
                            ? strtok($_SERVER['REQUEST_URI'], '?')
                            : '?show_comment_form=' . $song['id'] . '#song' . $song['id'];
                        ?>
                        <a href="<?= htmlspecialchars($formToggleUrl) ?>">
                            <button class="flex items-center space-x-1">
                        ...
                        <?php if (isset($_GET['show_comment_form']) && $_GET['show_comment_form'] == $song['id']): ?>
                            <form action="../actions/comment.php" method="post" class="mt-4">
                                <div class="w-full mb-4 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
                                    <input type="hidden" name="song_id" value="<?= $song['id'] ?>">
                                    <div class="px-4 py-2 bg-white rounded-t-lg dark:bg-gray-800">
                                        <label for="comment_text" class="sr-only">Your comment</label>
                                        <textarea name="comment_text" rows="4" class="w-full px-0 text-sm text-gray-900 bg-white border-0 dark:bg-gray-800 focus:ring-0 dark:text-white dark:placeholder-gray-400" placeholder="Write a comment..." required></textarea>
                                    </div>
                                    <input type="hidden" name="last_page" value="<?= htmlspecialchars($_SERVER['REQUEST_URI']) ?>">
                                    <div class="flex items-center justify-between px-3 py-2 border-t dark:border-gray-600 border-gray-200">
                                        <button type="submit" class="inline-flex items-center py-2.5 px-4 text-xs font-medium text-white bg-blue-700 rounded-lg hover:bg-blue-800">
                                            Post comment
                                        </button>
                                    </div>
                                </div>
