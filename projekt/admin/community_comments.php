...
$stmt = $db->query('SELECT 
    ...
    community_comments.edited,
    users.username,
    community_posts.title,
    community_posts.id AS post_id
FROM community_comments
INNER JOIN users ON community_comments.user_id = users.id
INNER JOIN community_posts ON community_comments.post_id = community_posts.id;
');
                ...
                <tbody>
                    <?php foreach ($data as $comment): ?>
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                            ...
                            <td class="px-6 py-4">
                                <?= $creator_username = $comment['username'] ?>
                            </td>
                            <td class="px-6 py-4">
                                    ...
                                    <div id="updateProductModal<?= $id ?>" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
                                        <div class="relative p-4 w-full max-w-4xl h-full md:h-auto">
                                            ...
                                            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                                ...
                                                <!-- Modal body -->
                                                <form action="updateCommunityComment.php" method="POST">
                                                    <input type="hidden" name="id" value="<?= $id; ?>">
                                                    <div class="grid gap-4 mb-4 sm:grid-cols- col-span-2">
                                                        <div>
                                                            <label for="text" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Comment text</label>
                                                            <textarea id="text" name="text" rows="4" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"><?= $text; ?></textarea>
                                                        </div>
                                                    </div>
                                                    <div class="grid gap-4 mb-4 sm:grid-cols-">
                                                        <div>
                                                            <input id="default-checkbox" type="checkbox" name="edited" id="edited" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                                            <label for="default-checkbox" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Is the comment edited</label>
                                                        </div>
                                                    </div>
                                                ...
                                                <form action="deleteCommunityComment.php" method="POST">
                                                    <input type="hidden" name="id" value="<?= $id; ?>">
