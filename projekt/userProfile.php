...
$stmt = $db->prepare("SELECT username, date_joined, profile_picture from users WHERE id = :id");
$stmt->execute([
    ':id' => $id
]);
    ...
    <section class="bg-white dark:bg-gray-900">
        <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 grid lg:grid-cols-2 gap-8 lg:gap-16">
            <div class="flex flex-col justify-center">
                <h1 class="mb-4 text-4xl font-extrabold tracking-tight leading-none text-gray-900 md:text-5xl lg:text-6xl dark:text-white"><?= $user['username'] ?></h1>
                <p class="mb-8 text-lg font-normal text-gray-500 lg:text-xl dark:text-gray-400">Date Joined Soundwave: <?= $user['date_joined'] ?></p>
            </div>
            <div>
                <div class="flex justify-center mt-4">
                    <figure class="max-w-lg">
                        <img class="h-auto max-w-full rounded-lg" src="<?= $user['profile_picture'] ?>" alt="error loading image">
                    </figure>
                </div>
            </div>
        </div>
